...
def cleaningData():
    ...
    '''
    # Read data
    df = pd.read_csv('/opt/survey_financial_ref_dataset_raw.csv')

    # Remove duplicate
    df = df.drop_duplicates()

    # Remaining columns names
    df.columns = [x.str.lower().str.replace(" ", "_") for x in df.columns]

    # Save the cleaned data to a new CSV file
    df.to_csv('/opt/survey_cleaned_data.csv', index=False)
...
def postToElasticsearch():
    ...
    '''
    # Read data
    df = pd.read_csv('/opt/survey_cleaned_data.csv')
